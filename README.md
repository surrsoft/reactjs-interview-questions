...
|6  | [When to use a Class Component over a Function Component?](#огда-использовать-классовый-компонент-а-когда-функциональный)                                                                                                       |
...
6. ### + Когда использовать классовый компонент, а когда функциональный?
...
8. ### + Что такое состояние (стейт) React?
    ...
    *Состояние* компонента - это объект, содержащий некоторую информацию, которая может меняться в течение жизни компонента. Мы всегда должны стараться сделать наше состояние как можно более простым и минимизировать количество компонентов с состоянием.
    ...
    Состояние похоже на props, но оно является приватным и полностью контролируется компонентом, т.е. оно недоступно для любого другого компонента, пока компонент-владелец не решит передать его.
...
9. ### What are props in React?
    ...
    *Props* - это входные данные компонентов. Это отдельные значения или объекты, содержащие набор значений, которые передаются компонентам при создании с использованием соглашения об именовании, аналогичного атрибутам HTML-тегов. Они представляют собой данные, передаваемые от родительского компонента к дочернему компоненту.
    ...
    Основное назначение пропсов в React заключается в обеспечении следующей функциональности компонентов:

    1. Передача кастомных данных в компонент.
    2. Запускать изменения состояния.
    ...
    Это имя `reactProp` (или любое другое, которое вы придумали) затем становится свойством, прикрепленным к нативному объекту React props, который изначально уже существует у всех компонентов, созданных с помощью библиотеки React.
